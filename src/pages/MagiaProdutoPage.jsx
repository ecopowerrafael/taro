...
export function MagiaProdutoPage() {
    ...
    return [
      {
        ...
        '@type': 'Product',
        name: getTranslatedField(spell, 'title'),
        description: getTranslatedField(spell, 'shortDescription') || getTranslatedField(spell, 'description'),
      ...
      },
      {
        '@context': 'https://schema.org',
        '@type': 'Service',
        name: `${getTranslatedField(spell, 'title')} com ${spell.consultantName}`,
        description: getTranslatedField(spell, 'shortDescription') || getTranslatedField(spell, 'description'),
        provider: {
          '@type': 'Person',
  ...
  if (!spell) {
    return (
      <div className="min-h-screen bg-mystic-black text-white overflow-x-hidden font-lato selection:bg-mystic-gold/30 selection:text-mystic-gold">
        ...
        <SacredGeometry />
        <PageShell title={t('nav.spells', 'Magias')} subtitle={t('spells.loading_details', 'Carregando detalhes do ritual.')}>
          <section className="rounded-3xl border border-mystic-gold/20 bg-black/25 p-10 text-center text-amber-100/75">
            {t('spells.loading_ritual', 'Carregando ritual...')}
          </section>
        </PageShell>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-mystic-black text-white overflow-x-hidden font-lato selection:bg-mystic-gold/30 selection:text-mystic-gold">
      <SeoHead
        title={`${getTranslatedField(spell, 'title')} | Magia personalizada na Astria`}
        description={
          getTranslatedField(spell, 'shortDescription') ||
          getTranslatedField(spell, 'description') ||
          `Conheca os detalhes da magia ${getTranslatedField(spell, 'title')} com ${spell.consultantName} na Astria.`
        }
        keywords={[
      ...
      <PageShell title={getTranslatedField(spell, 'title')} subtitle={t('spells.full_details_subtitle', 'Detalhes completos do ritual e do mentor responsável.')}>
        <section className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="grid gap-6">
            ...
            >
              <ArrowLeft size={16} />
              {t('spells.back_to_spells', 'Voltar para magias')}
            </Link>
            ...
            <div className="rounded-[2rem] border border-mystic-gold/15 bg-black/25 p-6 md:p-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-mystic-gold/20 bg-mystic-gold/5 px-4 py-2 text-xs uppercase tracking-[0.2em] text-mystic-goldSoft/85">
                <Sparkles size={14} />
                {t('spells.custom_ritual', 'Ritual Personalizado')}
              </div>

              <h1 className="mt-5 font-playfair text-4xl leading-tight text-white md:text-5xl">{getTranslatedField(spell, 'title')}</h1>

              {getTranslatedField(spell, 'shortDescription') ? (
                <p className="mt-4 max-w-3xl text-lg leading-relaxed text-amber-100/80">{getTranslatedField(spell, 'shortDescription')}</p>
              ) : null}
              ...
              <div className="mt-8 grid gap-6 md:grid-cols-[auto_1fr] md:items-center">
                ...
                <div className="text-center md:text-left">
                  <p className="text-xs uppercase tracking-[0.22em] text-amber-100/55">{t('spells.mentor_responsible', 'Mentor responsável')}</p>
              ...
              <div className="mt-8 border-t border-mystic-gold/15 pt-8">
                <p className="text-xs uppercase tracking-[0.22em] text-amber-100/55">{t('spells.full_description', 'Descrição completa')}</p>
                <p className="mt-4 whitespace-pre-line text-base leading-8 text-amber-100/82">{getTranslatedField(spell, 'description')}</p>
              </div>
            </div>
          </div>
          ...
          <aside className="h-fit rounded-[2rem] border border-mystic-gold/20 bg-[linear-gradient(180deg,rgba(197,160,89,0.09),rgba(0,0,0,0.18))] p-6 shadow-[0_25px_80px_rgba(0,0,0,0.35)] md:p-8 lg:sticky lg:top-28">
            <p className="text-xs uppercase tracking-[0.22em] text-amber-100/55">{t('spells.ritual_investment', 'Investimento do ritual')}</p>
            ...
            <p className="mt-3 text-sm leading-relaxed text-amber-100/72">
              {t('spells.payment_desc', 'Você escolhe PIX ou cartão de crédito no próximo passo, com confirmação vinculada à sua conta.')}
            </p>
            ...
            <div className="mt-8 rounded-2xl border border-mystic-gold/15 bg-black/20 p-5">
              <p className="text-xs uppercase tracking-[0.22em] text-amber-100/55">{t('spells.flow', 'Fluxo')}</p>
              <div className="mt-4 grid gap-4 text-sm text-amber-100/78">
                <p>{t('spells.flow_step_1', '1. Escolha a forma de pagamento e confirme o pedido.')}</p>
                <p>{t('spells.flow_step_2', '2. O mentor entra em contato se precisar alinhar informações adicionais.')}</p>
                <p>{t('spells.flow_step_3', '3. O ritual é conduzido conforme a intenção e a demanda solicitada.')}</p>
              </div>
            </div>
          </aside>
        </section>
